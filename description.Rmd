...
* `team_stats`: Team statistics of the first 20 games played by each team from season 2016-17 to 2019-20. We would use these data to define team performance during a specific season in the feature engineering step.
...
team_ws_plot = function(year){
  ...
  p = WL_record_ws %>%
    ...
    geom_text(aes(label=WS),color="white",size=2.5)+
...
player_ws_plot = function(){
  ...
  p = ws_stats_rank %>%
    ...
    ylab("Win shares rank")+
     labs(title = "Players with High Win Shares")
  print(p)
}
player_ws_plot()
